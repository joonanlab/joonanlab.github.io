...
AlphaGenome ([Cheng et al. 2025, Nature](https://doi.org/10.1038/s41586-025-10014-0)) uses deep learning to predict the effects of variants in noncoding regions. Deep learning models predicting noncoding variant effects have been developed over the past decade.

## Why Are Noncoding Variants So Hard to Study?

The opposite of noncoding is coding. Here "coding" refers to gene sequences that can become proteins, more precisely protein-coding sequences, and has nothing to do with programming. Coding sequences are intuitive. The flow from DNA to RNA to amino acids to proteins follows clear rules. A protein is determined by triplets of DNA bases. This is as deterministic as the sun rising in the east.
...
When organisms produce proteins, that is, when genes are transcribed, regulatory elements bind to the DNA sequence. This binding does not occur in a one-dimensional space. The DNA sequence folds and loops into three-dimensional configurations. Sequences that are physically distant can come together at a single "moment of transcription."
...
A different approach tried to bridge this gap: state space models like Mamba ([Gu & Dao 2023](https://arxiv.org/abs/2312.00752)). Mamba processes the sequence from left to right, passing information through a hidden state that summarizes prior context instead of attending over it. Its computational complexity is near-linear, and its advantages grow with sequence length (bi-directional Mamba also emerged, since DNA is a double helix).

The appeal for genomic problems was clear. DNA is sequential. Transcriptional regulation is the result of accumulated context along the sequence. Mamba represents this context through an evolving state rather than pairwise comparisons of all positions. In theory, it can process hundreds of thousands or even millions of base pairs in a single pass.
...
Against this backdrop, AlphaGenome made yet another architectural choice. It uses a UNet to compress long sequences into multi-resolution summaries and then reconstruct them. This reframes the "how long can we read" problem as one of spatial compression and restoration.

The UNet's core idea is straightforward. First, compress the sequence progressively to capture broad context. This gathers regulatory signals tens of thousands of base pairs apart into a single representation space. Then restore the original resolution, preserving local information through skip connections. Originally designed for medical imaging, where local lesions and global context must be seen at once, AlphaGenome adapted this structure for genomics.

The result is a model that asks what position the sequence block containing a variant occupies in the entire transcriptional regulatory network, not just whether the variant disrupts a motif. At least architecturally, that is what it aims to do.
...
What AlphaGenome predicts is "in the cellular state of K562, what functional signal might this sequence produce." That is a data limitation, not a modelling mistake. From a drug discovery perspective, the difference is critical.
...
Google will provide the AlphaGenome framework to various pharmaceutical companies. These companies will train it on their own high-quality proprietary data for drug discovery applications.

What decides the outcome is producing good data, and companies investing directly in that effort.
